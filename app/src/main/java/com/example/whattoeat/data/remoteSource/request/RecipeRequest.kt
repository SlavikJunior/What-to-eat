package com.example.whattoeat.data.remoteSource.request

import com.example.whattoeat.domain.models.support.*
import kotlinx.serialization.SerialName
import kotlin.collections.forEach
import kotlin.reflect.full.findAnnotation
import kotlin.reflect.full.memberProperties

sealed interface RecipeRequest {

    data class RecipeByIngredientsRequest(
    ...
    data class RecipeComplexRequest(
    ...
    data class RecipeFullInformationRequest(
    ...
    data class RecipeFullInformationBulkRequest(
        val ids: String, // Separated by ','
        val includeNutrition: Boolean = true // Включите в описание рецепта информацию о пищевой ценности на одну порцию.
    ): RecipeRequest

    data class RecipeSimilarRequest(
    ...
    @JvmInline
    value class RecipeSummaryRequest(
        @IdFromPath val id: Int // Идентификатор рецепта. не парметр - часть в path!!!
    ): RecipeRequest
}

fun RecipeRequest.toQueryMap(): Map<String, String> {
